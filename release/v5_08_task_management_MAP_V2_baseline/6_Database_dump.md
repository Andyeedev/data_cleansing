...
|------|----------|--------|-------------|
| `db/migration_engine_MAP_V2.dump` | migration_engine | PostgreSQL Custom (PGDMP) | Platform database (engine, platform, audit, core, reporting schemas) |
| `db/migration_source_MAP_V2.dump` | migration_source | PostgreSQL Custom (PGDMP) | Source database for validation |
| `db/migration_target_MAP_V2.dump` | migration_target | PostgreSQL Custom (PGDMP) | Target database for validation |
...
pg_dump -U postgres -d migration_engine -Fc --file=release/MAP_V2_task_management/db/migration_engine_MAP_V2.dump

# Source database - Custom format
pg_dump -U postgres -d migration_source -Fc --file=release/MAP_V2_task_management/db/migration_source_MAP_V2.dump

# Target database - Custom format
pg_dump -U postgres -d migration_target -Fc --file=release/MAP_V2_task_management/db/migration_target_MAP_V2.dump
```
...
pg_restore -U postgres -d migration_engine db/migration_engine_MAP_V2.dump

# Restore source database
pg_restore -U postgres -d migration_source db/migration_source_MAP_V2.dump

# Restore target database
pg_restore -U postgres -d migration_target db/migration_target_MAP_V2.dump
```

...
export const DistributionProfiles = () => {
  const { profiles, getChannelLabel } = useDistribution();

  return (
    <div>
      <h2 style={{ fontSize: '1.125rem', fontWeight: 600, color: 'var(--color-text-primary, #1f2937)', margin: '0 0 1rem 0' }}>Distribution Profiles</h2>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1rem' }}>
        {profiles.map((profile) => (
          <div key={profile.id} style={{ padding: '1rem', background: 'var(--color-surface, #ffffff)', borderRadius: '0.5rem', border: '1px solid var(--color-border, #e5e7eb)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.75rem' }}>
              <div>
                <div style={{ fontSize: '0.875rem', fontWeight: 600, color: 'var(--color-text-primary, #1f2937)' }}>{profile.name}</div>
            ...
            <div style={{ fontSize: '0.75rem', color: 'var(--color-text-secondary, #6b7280)' }}>Last updated: {profile.updatedAt}</div>
          </div>
        ))}
      </div>
    </div>

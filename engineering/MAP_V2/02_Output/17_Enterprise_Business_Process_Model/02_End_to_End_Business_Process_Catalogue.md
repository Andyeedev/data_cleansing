...
| **Current Implementation** | app.governance.decision_engine computes governance decisions; app.governance.risk_scoring calculates weighted risk scores; results stored in engine.migration_governance_status and engine.migration_risk_scores; auto post-execution |
...
| **Supporting Systems** | app.audit_export, reporting schema views |
...
| **Supporting Systems** | app.scoring_engine, reporting schema |
...
| **Current Implementation** | app.services.notification_service provides full CRUD API; platform.notifications stores notification records; platform.notification_preferences stores user preferences; subscribed to 19 event types |
| **Planned Integration** | Governance decisions to trigger notifications; Release Approval decisions to trigger notifications; Validation failures to trigger notifications; Workflow completions to trigger notifications |
...
| Field | Value |
|-------|-------|
| **Purpose** | Manage multi-tenant configuration and isolation |
| **Business Objective** | Support multi-tenant deployment with proper isolation |
| **Trigger** | Tenant lifecycle event (creation, configuration, deactivation) |
| **Inputs** | Tenant details, configuration |
| **Outputs** | Tenant configuration, tenant status |
| **Actors** | Administrator |
| **Primary Owner** | Administrator |
| **Supporting Roles** | Security Officer (isolation review) |
| **Start Event** | Tenant Event |
...
| **Supporting Systems** | core.tenants, JWT tenant_id |
...
| Field | Value |
|-------|-------|
| **Purpose** | Onboard new customer organisations to the platform |
| **Business Objective** | Enable new customers to start using the platform |
| **Trigger** | New customer organisation signs up |
| **Inputs** | Customer registration details, organisation information |
| **Outputs** | Provisioned tenant, initial administrator account, platform access |
| **Actors** | Administrator, Security Officer |
| **Primary Owner** | Administrator |
| **Supporting Roles** | Security Officer (security review), DevOps (provisioning) |
| **Start Event** | Customer Registration |
| **End Event** | Platform Activated |
| **Business Rules** | Customer must complete registration; Organisation must be created; Tenant must be provisioned; Initial administrator must be created; Licence/subscription must be allocated; Initial configuration must be completed; Welcome notifications must be sent; Platform must be activated |
| **Dependencies** | Tenant Management, User Lifecycle, Authentication |
| **Upstream Processes** | — |
| **Downstream Processes** | Tenant Management, User Lifecycle, Authentication |
| **Related Business Capabilities** | Customer Onboarding (5.5) |
| **Supporting Systems** | Customer registration system, tenant provisioning, user management |
...
| **Business Rules** | Credentials must be validated; JWT tokens must be issued; Failed logins must be logged; Sessions must be managed; MFA must be enforced if configured; Refresh tokens must be managed; Logout must invalidate session |
...
| **Activities** | Login, credential validation, MFA (if implemented), JWT generation, refresh token lifecycle, session management, logout, authentication failure handling |
...
*This catalogue is part of the Enterprise Business Process Model (Prompt 17 v2.1). All findings are based on source code analysis — no code was modified.*
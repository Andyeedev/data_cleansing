...
class DatabricksAdapter(ConnectionAdapter):
    """Databricks adapter implementing the ConnectionAdapter interface."""
    ...
    def connect(self, config: DatabricksConfig) -> None:
    ...
    def test_connection(self, config: DatabricksConfig) -> ConnectionTestResult:
        ...
        if self._connection:
            try:
                self._pool_manager.release_by_config(
                    self._config, "databricks", self._connection
                )
            except Exception:
                pass
            finally:

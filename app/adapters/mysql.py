...
class MySQLAdapter(ConnectionAdapter):
    """MySQL adapter implementing the ConnectionAdapter interface."""
    ...
    def connect(self, config: MySQLConfig) -> None:
    ...
    def list_tables(self, schema: str = None) -> List[TableInfo]:
        ...
        """
        rows = self.fetch_all(query, (schema or self._config.database,))
    ...
    def test_connection(self, config: MySQLConfig) -> ConnectionTestResult:
        ...
        if self._connection:
            try:
                self._pool_manager.release_by_config(
                    self._config, "mysql", self._connection
                )
            except Exception:
                pass
            finally:

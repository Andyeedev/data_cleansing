...
class SQLServerAdapter(ConnectionAdapter):
    ...
    def connect(self, config: SQLServerConfig) -> None:
    ...
    def list_tables(self, schema: str = None) -> List[TableInfo]:
        ...
        target_schema = schema or self._config.schema or "dbo"
        query = """
    ...
    def test_connection(self, config: SQLServerConfig) -> ConnectionTestResult:
        ...
        if self._connection:
            try:
                self._pool_manager.release_by_config(
                    self._config, "sqlserver", self._connection
                )
            except Exception:
                pass
            finally:

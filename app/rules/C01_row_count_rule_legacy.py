...
class RowCountRule(BaseRule):
    ...
    def execute(self):
        ...
        source_count = self.source_db.adapter.get_row_count(
            self.parameters["source_schema"],
            self.parameters["source_table"]
        )

        target_count = self.target_db.adapter.get_row_count(
            self.parameters["target_schema"],
            self.parameters["target_table"]
        )
